...
app.controller('ControlEstadisticas', function($scope, $http, $state,jwtHelper, $auth, $timeout, ServicioABM) {
  $scope.flagLogueado = false;
  ...
  $scope.ventasPorEmpleado = function(){
    var listaEmpleados = [];
    var cantidadVentasEmpleado = [];
    var ventas = [];

    ServicioABM.traerVentasPorEmpleado('transacciones/ventasporempleado/', '2').then(
      function(rta){
        ventas = rta.data;
        $("#loadingModal").modal('hide');
      },function(error){
